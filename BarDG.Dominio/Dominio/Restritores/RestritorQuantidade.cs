...
namespace BarDG.Dominio.Restritores
{
    ...
    public class RestritorQuantidade : IRestritor
    {
        public RestritorQuantidade()
        {
            ItemIdQuantidade = new Dictionary<int, int>();
        }
        public RestritorQuantidade(Dictionary<int, int> itemIdQuantidade)
        {
        ...
        public bool EhValido(Comanda comanda, Item item)
        {
            ...
            var quantidadeLimite = ItemIdQuantidade
                .FirstOrDefault(x => x.Key == item.Id).Value;
